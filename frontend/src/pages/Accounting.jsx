    ...
    return (
        <div className="app-layout">
            ...
            <main className="page-content">
                ...
                {/* ── RESUMEN ── */}
                {!loading && !error && tab === 'resumen' && data && data.totals && (
                    <>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
                            ...
                            <MetricCard label="Beneficio bruto"  value={fmt(data.totals?.beneficio_bruto)} color={(data.totals?.beneficio_bruto ?? 0) >= 0 ? 'var(--secondary)' : 'var(--error)'}
                ...
                {/* ── LIBRO ── */}
                {!loading && !error && tab === 'libro' && data && data.resumen && (
                    <>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
                            ...
                            <MetricCard label="Beneficio neto"  value={fmt(data.resumen?.beneficio_neto)} color={(data.resumen?.beneficio_neto ?? 0) >= 0 ? 'var(--primary)' : 'var(--error)'} />
                            <MetricCard label="Nº facturas"     value={data.resumen?.num_facturas ?? 0}   color="var(--text-primary)" />
                        </div>
                                ...
                                <tbody>
                                    {(!data.ingresos || data.ingresos.length === 0) ? (
                                        ...
                                        <tr key={i}>
                                            ...
                                            <td style={{ color: 'var(--text-secondary)' }}>{r.fecha ? new Date(r.fecha).toLocaleDateString('es-ES') : '—'}</td>
                                            <td>{r.cliente}</td>
                                            <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(r.base_imponible)}</td>
                ...
                {/* ── MODELO 130 ── */}
                {!loading && !error && tab === 'm130' && data && data.casillas && (
                ...
                {/* ── MODELO 303 ── */}
                {!loading && !error && tab === 'm303' && data && data.casillas && (
                    <div style={{ maxWidth: 680 }}>
                        <div className="alert alert-info" style={{ marginBottom: 20 }}>
